  ...
  <div class="admin-roles">
    ...
    <el-card shadow="hover" class="card-bottom card-header-border-none">
      ...
      <el-tree
        class="roles-tree"
        :data="menus"
        :default-checked-keys="menusDefaultCheckedKeys"
        show-checkbox
        default-expand-all
        node-key="id"
        ref="rolesTree"
...
import { editRole, getRole } from '@/api/admin/roles';
...
export default {
  ...
  methods: {
    ...
    async getRole() {
      try {
        const res = await getRole(this.roleForm.id);
        if (!res.success) {
          this.$message.error(res.message || '数据加载失败，请重试');
        } else {
          const { name, status, menus } = res.data;
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          try {
            ...
            if (!res.success) {
              this.$message.error(res.message || '编辑失败，请重试');
            } else {
